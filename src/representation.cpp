...
bool MakeAMove(Situation & situation, const Movement move){
    ...
    // 3. 更新子力价值
    situation.value[situation.current_player] -= pieces_value[situation.current_player][PIECE_NUM_TO_TYPE[piece_id_from]][move.from];
    situation.value[situation.current_player] += pieces_value[situation.current_player][PIECE_NUM_TO_TYPE[piece_id_from]][move.to];

    // 4. 移动棋子
    // 4.1 如果是吃子着法
    if(piece_id_to != 0){
        // 4.1.1 更新子力价值
        situation.value[ColorOfPiece(piece_id_to)] -= pieces_value[ColorOfPiece(piece_id_to)][PIECE_NUM_TO_TYPE[piece_id_to]][move.to];
        // 4.1.2 只更新起始格的位行位列
...
void UnMakeAMove (Situation & situation){
    ...
    // 3. 更新子力价值
    situation.value[ColorOfPiece(piece_id_from)] -= pieces_value[ColorOfPiece(piece_id_from)][PIECE_NUM_TO_TYPE[piece_id_from]][move.to];
    situation.value[ColorOfPiece(piece_id_from)] += pieces_value[ColorOfPiece(piece_id_from)][PIECE_NUM_TO_TYPE[piece_id_from]][move.from];
...
int IfProtected(int player, const int to, const Situation & situation, int exp){
    ...
    for(int i = player_flag + 0; i <= player_flag + 0; i ++){
        ...
        if (from && (LEGAL_POSITION[player][to] & POSITION_MASK[0]) && (LEGAL_MOVE[to - from + 256] == 1))
            return true;
    }
    ...
    for(int i = player_flag + 1; i <= player_flag + 2; i ++){
        from = situation.current_pieces[i];
        if (from && (LEGAL_POSITION[player][to] & POSITION_MASK[1]) && (LEGAL_MOVE[to - from + 256] == 2))
            return true;
    }
    ...
    for(int i = player_flag + 3; i <= player_flag + 4; i ++){
        from = situation.current_pieces[i];
        if(from && (LEGAL_POSITION[player][to] & POSITION_MASK[2]) && (LEGAL_MOVE[to - from + 256] == 3) && situation.current_board[(to + from) >> 1] == 0)
            return true;
    }
    ...
    for(int i = player_flag + 5; i <= player_flag + 6; i ++){
        from = situation.current_pieces[i];
        int horse_leg = HORSE_LEGAL_MOVE[to - from + 256] + from;
        if(from && horse_leg != from && situation.current_board[horse_leg] == 0)
            return true;
    }
